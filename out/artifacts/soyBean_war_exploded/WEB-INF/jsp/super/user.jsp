...
<head>
    ...
    <script type="text/javascript">
        function upuser() {
            window.location.href = "${pageContext.request.contextPath}/admin/excelUser";
        }
    </script>
        ...
        <div class="col overflow-auto main-box">
            ...
            <div class="row flex-column m-2">
                ...
                <div class="col mt-2">
                    <div class="card px-3">
                        <div class="card-body">
                            <%--<form action="${pageContext.request.contextPath}/admin/delectUser" method="post">--%>
                            <div>
                                <a href="${pageContext.request.contextPath}/admin/toAddUser"
                                   class="btn btn-success" data-toggle="modal"
                                   data-target="#addModel"><i class="bi bi-person-fill-add"></i>添加用户</a>
                                <button type="button" class="btn btn-danger" onclick="deleteUserAll()"><i
                                        class="bi bi-trash-fill"></i>批量删除
                                </button>
                                <a type="button" class="btn btn-success" onclick="upuser()">
                            ...
                            <div class="table-responsive">
                                ...
                                <table class="table small text-muted table-bordered mt-3">
                                    <tr>
                                        <th class="col-p-3"><input type="checkbox" id="alluser"
                                                                   onclick="alluserClick()"></th>
                                    ...
                                    <c:forEach var="user" items="${pageInfo.list}" varStatus="pages">
                                        <tr>
                                            <td><input type="checkbox" id="usids" name="usids" value="${user.usid}"
                                                       onclick="userClick()"></td>
                                            <td>${user.username}</td>
                                            <td>${user.password}</td>
                                            <td>${user.name}</td>
                                            <td>${user.sex}</td>
                                            ...
                                            <td>
                                                ...
                                                <div class="btn-group">
                                                    <button type="button" class="btn btn-warning btn-sm"
                                                            ...
                                                            data-toggle="modal"
                                                            data-target="#updateUserModal">修改
                                                    </button>&nbsp;&nbsp;
                            ...
                            <!-- 添加模态框 Model -->
                            <form method="post"
                                  class="form-horizontal" action="${pageContext.request.contextPath}/admin/addUser">
                                <div class="modal fade" id="addModel" tabindex="-1" aria-labelledby="AddUser"
                                     aria-hidden="true">
                                    <div class="modal-dialog">
                                        <div class="modal-content">
                                            <div class="modal-header">
                                                <h5 class="modal-title" id="AddUser">添加用户信息</h5>
                            ...
                            <!-- 修改模态框 Model -->
                            <form method="post"
                                  class="form-horizontal" action="${pageContext.request.contextPath}/admin/update">
                                <div class="modal fade" id="updateUserModal" tabindex="-1" aria-labelledby="UPdateUser"
                                     aria-hidden="true">
                                    <div class="modal-dialog">
                                        <div class="modal-content">
                                            <div class="modal-header">
                                                <h5 class="modal-title" id="UPdateUser">修改用户信息</h5>
                                                <button type="button" class="close" data-dismiss="modal"
                                                        aria-label="Close">
                                                    <span aria-hidden="true">&times;</span>
                                                </button>
                                            ...
                                            <div class="modal-body ">
                                                <input type="hidden" name="usid" id="ususid" value="${user.usid}">
                                                <div class="form-group row ">
                                                    <label for="ususername"
                                                           class="col-3 col-form-label">用户账号：</label>
                                                    <div class="col-7"><input type="text" class="form-control"
                                                                              id="ususername" name="username"
                                                                              value="" required
                            ...
                            <!-- 删除模态框示例（Modal） -->
                            <form method="post" action="${pageContext.request.contextPath}/admin/delectById"
                                                ...
                                                <form class="form-horizontal" role="form">
                                                    <div class="form-group">
                                                        <div class="col-sm-9">
                                                            ...
                                                            <input
                                                                    type="hidden" class="form-control" id="usid"
                                                                    name="usid" placeholder="">
                                                            <input
                                                                    type="hidden" class="form-control" id="usrusername"
                                ...
                                <nav aria-label="Page navigation example">
                                    <ul class="pagination justify-content-end m-0">
                                        <li <c:if test="${pageInfo.isFirstPage}">class="page-item disabled"</c:if>>
                                            <a href="${pageContext.request.contextPath}/admin/allUser?pn=1"
                                        ...
                                        <li <c:if test="${!pageInfo.hasPreviousPage}">class="page-item disabled"</c:if>>
                                            <a href="${pageContext.request.contextPath}/admin/allUser?pn=${pageInfo.prePage==0?1:pageInfo.prePage}"
                                        ...
                                        <c:forEach begin="0" items="${pageInfo.navigatepageNums}"
                                            ...
                                            <c:if test="${pageNum != pageInfo.pageNum }">
                                                <li class="page-item"><a
                                                        href="${pageContext.request.contextPath}/admin/allUser?pn=${pageNum}"
                                                        class="page-link">${pageNum}</a></li>
                                            </c:if>
                                        </c:forEach>
                                        <%--跳到下一页 判断是否有下一页 则不可点击后跳 class="disabled" --%>
                                        <li <c:if test="${!pageInfo.hasNextPage}">class="page-item disabled"</c:if>>
                                            <a href="${pageContext.request.contextPath}/admin/allUser?pn=${pageInfo.nextPage==pageInfo.pages?pageInfo.pages:pageInfo.nextPage}"
                                               class="page-link">下一页</a>
                                        </li>
                                        <li <c:if test="${pageInfo.isLastPage}">class="page-item disabled"</c:if>>
                                            <a href="${pageContext.request.contextPath}/admin/allUser?pn=${pageInfo.pages}"
...
<script type="text/javascript">
    ...
    $('#updateUserModal').on('show.bs.modal', function (event) {
        ...
        modal.find('#ususid').val(usid)
        modal.find('#ususername').val(username)
        modal.find('#uspassword').val(password)
        modal.find('#usname').val(name)
        ...
        modal.find('#usemail').val(email)
        modal.find('#usphone').val(phone)
    });
...
<script type="text/javascript">
    function deleteUserAll() {
        ...
        $.ajax({
            type:"post",
            url:"${pageContext.request.contextPath}/admin/delectUser",
